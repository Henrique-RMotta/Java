    ...
    public class Main extends ApplicationAdapter {
        ...
        private void logic() {
            ...
            if (inimigosGerados < numInimigosRodada) {
                if (Timer >= valorTimer) {
                    createInimigos();
                    inimigosGerados++;
                    Timer = 0;
                    ...
                    if (valorTimer < 0.3f) {
                        valorTimer = 0.3f; // Limite máximo de velocidade
                    }
                }
            }
            System.out.println(numInimigos);
            if (inimigosGerados == numInimigosRodada && inimigos.size == 0) {
                if (Timer >= 10f) {
                    int proximaRodada = numRodadas + 1;

                    Rodada.setRodada(proximaRodada);
                    Rodada.setNumInimigos(proximaRodada * 10);

                    inimigosGerados = 0;  // Zera a contagem para a nova rodada começar do zero
                    valorTimer = 1f;
                    Timer = 0;
                }
            }
